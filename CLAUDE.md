...
(근거: `docs/PRD.md` §1, §3. 디자인: `docs/design_reference.html`)
...
2. **비목표 위반 아님** — `docs/PRD.md` §3.2를 어기지 않는가? (Windows/Linux, 자체 백엔드, 협업, 프로젝트 관리 고급 기능, **달력 그리드 뷰**, API 미지원 기능의 임의 구현 = 전부 금지)
3. **API 제약 존중** — `docs/PRD.md` §8.4를 어기지 않는가? (마감일=날짜 단위만, 서브태스크=1단계만, 반복/리마인더/푸시 없음, 계정 간 이동=재생성+삭제, 폴링 기반)
...
## 3. 디자인 원칙 (docs/design_reference.html 준수)
...
- **남은 것:** Google 검증(sensitive 스코프 `tasks`) 진행 중. 동의 화면은 **Production(정식) 모드** —
  임의 사용자 로그인 가능하나 로그인 시 "확인되지 않은 앱" 경고가 뜨고, 검증 전 연결 계정 **약 100명 상한**.
  (Testing 모드의 7일 리프레시 만료는 **해당 없음**.) 실계정 검증 항목은 `docs/oauth_setup.md` 하단 참조.
