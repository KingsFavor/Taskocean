...
### 주의사항
- **Testing 모드의 리프레시 토큰은 7일 후 만료**된다. 검증(verification) 전까지는
  주 1회 재로그인 배너가 뜨는 게 정상. 테스트 사용자는 최대 100명.
- `tasks` 스코프는 **민감(sensitive) 스코프** — 일반 공개(Production 전환) 전
  Google 검증 필요. 최장 지연 요인이므로 TestFlight 외부 배포 전에 조기 착수 (PRD §10, §13).
